...
public class Activity_Form extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        cl = (ConstraintLayout) findViewById(R.layout.activity_form);
...
*/
    ...
    public boolean onContextItemSelected(MenuItem item){
        ...
        if(item.getTitle().equals("N"))
        {
            ...
            {
                if(date.getTime()<(time[0]+2000)) time[1] = date.getTime();
                else contw = 0;
            }
            else if(contw ==3)
            {
                if(date.getTime()<(time[1]+2000)) time[2] = date.getTime();
                else contw = 0;
            }
            if(contw == 3){
                time[0] = 0;
        ...
        else if(item.getTitle().equals("E"))
        {
            ...
            {
                if(date.getTime()<(time1[0]+2000)) time1[1] = date.getTime();
                else conte = 0;
            }
            else if(conte ==3)
            {
                if(date.getTime()<(time1[1]+2000)) time1[2] = date.getTime();
                else conte = 0;
            }
            if(conte == 3){
                time1[0] = 0;
        ...
        else if(item.getTitle().equals("W"))
        {
            ...
            {
                if(date.getTime()<(time2[0]+2000)) time2[1] = date.getTime();
                else contn = 0;
            }
            else if(contn ==3)
            {
                if(date.getTime()<(time2[1]+2000)) time2[2] = date.getTime();
                else contn = 0;
            }
            if(contn == 3){
                time2[0] = 0;
